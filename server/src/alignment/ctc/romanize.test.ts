...
describe('cyrillic transliteration', () => {
  it('follows the documented BGN/PCGN simplification', () => {
    ...
    expect(romanize('дядя Веня', 'ru')).toBe('dyadyavenya');
  });
});
...
describe('number expansion', () => {
  ...
  it('spells English integers 0-9999', () => {
    ...
    expect(en('1987')).toBe('onethousandninehundredeightyseven');
  ...
  it('expands ampersands', () => {
    expect(en('Tom & Ada')).toBe('tomandada');
    expect(romanize('Tom & Ada', 'de')).toBe('tomundada');
  });
...
describe('abbreviations', () => {
  it('expands English titles and common forms', () => {
    ...
    expect(romanize('Dr. Holm', 'en')).toBe('doctorholm');
  ...
  it('requires the guard conditions before expanding a risky key', () => {
    ...
    expect(romanize('No. 5 Wrenfield Lane', 'en')).toBe('numberfivewrenfieldlane');
