...
function mkDoc(over: { title?: string; author?: string } = {}): PortableAlignment {
  return buildAlignmentDocument({
    ...
    ebook: {
      title: over.title ?? 'The Reedbank Almanac',
      author: over.author ?? 'Corwin Ashgrove',
      language: 'en',
      spineCount: CHAPTERS,
      sizeBytes: 428_900,
    ...
    audio: {
      title: over.title ?? 'The Reedbank Almanac',
      narrator: 'Mirela Stanhope',
      trackDurationsMs: [1_811_000, 1_736_500, 1_702_250],
    },
    alignment: {
      version: 2,
...
describe('round-trip', () => {
  ...
  it('derives the counts and total duration from the inputs they must agree with', () => {
    ...
    expect(doc.audio.durationMs).toBe(1_811_000 + 1_736_500 + 1_702_250);
...
describe('buildAlignmentDocument', () => {
  it('accepts a fingerprint the caller already has instead of the sentence ids', () => {
    ...
    const fromFingerprint = buildAlignmentDocument({
      ...
      writtenAt: '2026-09-12T10:00:00.000Z',
      ebook: {
        title: 'The Reedbank Almanac',
        author: 'Corwin Ashgrove',
        language: 'en',
        spineCount: CHAPTERS,
        sizeBytes: 428_900,
      ...
      audio: {
        ...
        trackDurationsMs: [1_811_000, 1_736_500, 1_702_250],
        trackCount: 3,
        durationMs: 5_249_750,
        timelineFingerprint: timelineFingerprint([1_811_000, 1_736_500, 1_702_250]),
      },
...
describe('alignmentFileName', () => {
  ...
  it('drops punctuation that a filesystem or a shell would object to', () => {
    const name = alignmentFileName({
      author: 'Ashgrove, C.W.',
      title: 'Dr. Wren & Mr. Tallow: A "Novel"? / v2 *',
      pairKey: KEY,
    });
    expect(name).toBe(
      `Ashgrove C W - Dr Wren Mr Tallow A Novel v2 [9f2c1a7b3d5e]${ALIGNMENT_FILE_EXT}`,
    );
  ...
  it('falls back to a readable placeholder when a slug would be empty', () => {
    ...
    expect(alignmentFileName({ author: 'נוצה', title: '夜の灯台', pairKey: KEY })).toBe(
      `Unknown Author - Untitled [9f2c1a7b3d5e]${ALIGNMENT_FILE_EXT}`,
    );
  });
...
describe('fingerprints', () => {
  ...
  it('survives a retag: 40 ms of ffprobe disagreement is not a different audiobook', () => {
    ...
    expect(timelineFingerprint([1_811_000, 1_736_500])).toBe(
      timelineFingerprint([1_811_040, 1_736_460]),
    );
  });

  it('does not survive a real edit: 40 s more audio is a different timeline', () => {
    expect(timelineFingerprint([1_811_000, 1_736_500])).not.toBe(
      timelineFingerprint([1_851_000, 1_736_500]),
    );
  });
  ...
  it('derives a pair key that moves with either side', () => {
    ...
    const a = timelineFingerprint([1_811_000]);
