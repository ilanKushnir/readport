...
- **`standard`** (the default) samples the narration on a schedule and
  ...
  LXC with `VX_ALIGN_THREADS=4`): an hour-long book in a little over a minute,
...
```json
{
  ...
  "audio": { "title": "…", "narrator": "…", "trackCount": 4,
             "durationMs": 3720000, "trackDurationsMs": [ … ],
...
times narration in Cyrillic as happily as in English, and unvocalized Hebrew
