...
All measured on the same target server (a 4-CPU LXC) with the same hour-long
human-narrated audiobook of some nine hundred ebook sentences. They are not benchmarks of
...
| Wall clock, hour-long book            | 72 s                 | 150 s                  |
...
- **Un-narrated matter becomes a gap, not an error.** Title pages,
  ...
  about 94% of sentences got timings; the rest were gaps or too far from an
  anchor to trust.
- **Abridged editions are refused,** not silently half-aligned. A softer
  ...
  ratio on a matching pair was about 0.94.)
- **Digits and abbreviations are not in the model's alphabet.** The narrator
  ...
  9999, years read as digit pairs ("1987" spells as a cardinal, the narrator
  probably said "nineteen eighty seven"), Roman numerals, times, phone
