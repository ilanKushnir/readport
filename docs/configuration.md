...
| `RP_SETUP_TOKEN`           | unset (setup is open)              | Locks first-run setup: when set, creating the admin account requires it. Leave unset and the first person to open a server with no accounts creates the admin - which is what you want at home, and not what you want on a public URL with no gate in front. Never generated; consumed when the admin exists.                              |
...
| `RP_CLIENT_IP_HEADER`      | unset                              | Header naming the real client for **rate limits only** (`cf-connecting-ip` behind a Cloudflare tunnel). Read only from the peers in `RP_CLIENT_IP_SOURCES`; never an identity or an authorization input. Use when a hop sits further out than `RP_TRUST_PROXY` can reach, which otherwise collapses every internet client into one bucket. |
| `RP_CLIENT_IP_SOURCES`     | unset                              | Comma-separated IPs/CIDRs whose TCP peer may set `RP_CLIENT_IP_HEADER`. Checked on the socket, never on a forwarded header. Empty means the header is ignored and a warning is logged (fail closed).                                                                                                                                       |
