...
export interface AudioSupport {
  ...
  supported: boolean;
  /**
   * Honest user-facing explanation when unsupported, as a catalog key whose
   * `{format}` argument is `format`: `t(reason, { format })`.
   */
  reason: MessageKey | null;
  /** The format the reason names, upper-cased for display. */
  format: string | null;
}
...
export function audioFormatSupport(format: string, probe?: (mime: string) => string): AudioSupport {
  ...
  if (!mime) return { level: 'unknown', supported: true, reason: null, format: null };
  let canPlay: string;
  try {
    if (probe) {
      canPlay = probe(mime);
      ...
      return { level: 'unknown', supported: true, reason: null, format: null };
    }
  } catch {
    return { level: 'unknown', supported: true, reason: null, format: null };
  }
  if (canPlay === 'probably' || canPlay === 'maybe') {
    return { level: canPlay, supported: true, reason: null, format: null };
  }
  return {
    level: 'unsupported',
    supported: false,
    reason: 'library.book.audioUnsupported',
...
export function bookAudioSupport(
  ...
  return { level: 'probably', supported: true, reason: null, format: null };
}
